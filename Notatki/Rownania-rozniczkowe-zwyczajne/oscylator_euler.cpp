 ...
 * Przykład: rozwiązywanie równania ruchu oscylatora harmonicznego metodą Eulera
 */
...
rozwiążemy numerycznie, posługując się metodą Eulera.
...
Program zapisze wyniki obliczeń w pliku oscylator_euler.dat w następującym formacie:

Oscylator harmoniczny - metoda Eulera: [wartości parametrów]
t0 x0 v0 x_exact0
t1 x1 v1 x_exact1
t2 x2 v2 x_exact2
...
...
public:
	...
	*/	
	...
	// Konstruktor.
	HarmonicOscillatorEuler(double omega, double x0, double v0, double tmax, double dt)
...
public:	
	// Metoda SolveEuler: rozwiązywanie równania różniczkowego oscylatora metodą Eulera.
	void SolveEuler();
};

double HarmonicOscillatorEuler::Fx(double t, double x, double v)
{
	return v;
}

double HarmonicOscillatorEuler::Fv(double t, double x, double v)
{
	return (- std::pow(omega, 2) * x);
}

double HarmonicOscillatorEuler::SolveExact(double t)
{
	double A = v0 / omega;
	double B = x0;
...
void HarmonicOscillatorEuler::SolveEuler()
{
...
int main(int argc, char *argv[])
{
	...
	HarmonicOscillatorEuler osc {omega, x0, v0, tmax, dt};
	osc.SolveEuler();
	
	// Zapisujemy wyniki obliczeń do pliku.
	std::ofstream results("oscylator_euler.dat");
	
	results << "Oscylator harmoniczny - metoda Eulera: "
		<< "$\\omega $ = " << osc.omega << ", "
		<< "$x_0$ = " << osc.x0 << ", "
		<< "$v_0$ = " << osc.v0 << ", "
